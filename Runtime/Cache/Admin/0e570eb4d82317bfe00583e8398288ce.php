	...
	<div class="g-wrap">
				...
				<table width="100%">
				    ...
				    <tbody>
						<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('AuthManager/changeStatus?method=forbidGroup&id='.$vo['id']);?>" class="J_ajax_get">禁用</a>
								<?php else: ?>
								<a href="<?php echo U('AuthManager/changeStatus?method=resumeGroup&id='.$vo['id']);?>" class="J_ajax_get">启用</a><?php endif; ?>
								<a href="<?php echo U('AuthManager/changeStatus?method=deleteGroup&id='.$vo['id']);?>" class="confirm J_ajax_get">删除</a>
				                </td>
		...
		<div class="btn_wrap">
			<div class="btn_wrap_pd">
		        ...
		        <a id="add-group" class="u-btn" href="<?php echo U('createGroup');?>">新 增</a>
		        <a url="<?php echo U('changestatus',array('method'=>'resumeGroup'));?>" class="u-btn J_ajax_post" target-form="ids" >启 用</a>
		        <a url="<?php echo U('changestatus',array('method'=>'forbidGroup'));?>" class="u-btn J_ajax_post" target-form="ids" >禁 用</a>
		        <a url="<?php echo U('changestatus',array('method'=>'deleteGroup'));?>" class="u-btn J_ajax_post confirm" target-form="ids" >删 除</a>
		    </div>
		</div>
		<!-- 分页 -->
	    <div class="page">
