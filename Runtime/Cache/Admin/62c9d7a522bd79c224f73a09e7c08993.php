...
<body style="" screen_capture_injected="true">
    ...
    <div class="g-wrap">
        ...
        <div class="u-tab">
            <ul class="cc">
              <li class="current"><a href="javascript:;">模型列表</a></li>
            </ul>
        </div>
        <!-- 数据表格 -->
        <div class="table_list">
            ...
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
                        ...
                        <a href="<?php echo U('model/setstatus?ids='.$vo['id'].'&status='.abs(1-$vo['status']));?>" class="J_ajax_get"><?php echo (show_status_op($vo["status"])); ?></a>
                        <a href="<?php echo U('model/edit?id='.$vo['id']);?>">编辑</a>
                        <a href="<?php echo U('model/del?ids='.$vo['id']);?>" class="confirm J_ajax_get">删除</a>
                    </td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php else: ?>
                <td colspan="7" class="input-center"> aOh! 暂时还没有内容! </td><?php endif; ?>
            </tbody>
            </table> 
        </div>
        <div class="btn_wrap">
            <div class="btn_wrap_pd">
                ...
                <a class="btn btn_submit" href="<?php echo U('Model/add');?>">新 增</a>
                <button class="btn btn_submit J_ajax_post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>1));?>">启 用</button>
                <button class="btn btn_submit J_ajax_post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>0));?>">禁 用</button>
                <a class="btn btn_submit" href="<?php echo U('Model/generate');?>">生 成</a>
            </div>
        </div>
        <!-- 分页 -->
        <div class="page">
