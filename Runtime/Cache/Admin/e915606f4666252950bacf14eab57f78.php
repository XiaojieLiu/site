...
<body style="" screen_capture_injected="true">
	...
	<script type="text/javascript" src="/office/Public/static/uploadify/jquery.uploadify.min.js"></script>
	<div class="g-wrap">
		<ul class="u-tab">
								...
								<?php case "5": ?><td width="88%">
										<div class="controls">
											...
											$("#upload_picture_<?php echo ($config["name"]); ?>").uploadify({
										        "height"          : 30,
										        "swf"             : "/office/Public/static/uploadify/uploadify.swf",
											...
											function uploadPicture<?php echo ($config["name"]); ?>(file, data){
										        ...
										        if(data.status){
										        	...
										        	src = data.url || '/office' + data.path
										        	$("#cover_id_<?php echo ($config["name"]); ?>").parent().find('.upload-img-box').html(
										        		'<div class="upload-pre-item"><img src="/office' + src + '" width="120" height="75"/></div>'
										        	);
										        } else {
