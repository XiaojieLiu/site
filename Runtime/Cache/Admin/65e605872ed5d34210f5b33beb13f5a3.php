...
<body style="" screen_capture_injected="true">
	...
	<div class="g-wrap u-table">
		...
		<div class="u-tab">
		    <ul class="cc">
		      ...
		      <li><a target="_self" href="<?php echo U('User/add');?>">新增用户</a></li>
		    </ul>
		</div>
	    <div class="search-form search_type cc mb10">
	      	<div class="mb10"> 
	        	<input type="text" name="nickname" class="input" value="<?php echo I('nickname');?>" placeholder="请输入用户昵称或者ID">
	        	<a id="search" url="<?php echo U('index');?>" class="btn" value="搜索">搜索</a>
	      	</div>
	    </div>
	    <!-- 数据列表 -->
	    <div class="table_list">
	      <table width="100%">
	        <thead>
	        	...
	        	<?php if(is_array($_list)): foreach($_list as $key=>$vo): ?><tr>
						...
						<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="J_ajax_get">禁用</a>
							<?php else: ?>
							<a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>" class="J_ajax_get">启用</a><?php endif; ?>
		                </td>
	      	...
	      	<div class="btn_wrap">
		        <div class="btn_wrap_pd">
			        ...
			        <button class="btn J_ajax_post" url="<?php echo U('User/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
		            <button class="btn J_ajax_post" url="<?php echo U('User/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
		            <button class="btn J_ajax_post confirm" url="<?php echo U('User/changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
		        </div>
	    	</div>
	    </div>
	</div>
	...
	<script src="/office/Public/static/thinkbox/jquery.thinkbox.js"></script>
